...
(function () {
  ...
  // Choice — segmented buttons for <=3 options, a friendly dropdown beyond that.
  // options: array of { value, label, sub? }. Preserves the original value type.
  function Choice({ value, options, onChange, cols }) {
    if (options.length <= 3) {
      return (
        <div className={"seg cols-" + (cols || options.length)}>
          {options.map((o) => (
            <button key={String(o.value)} className={"seg-btn" + (value === o.value ? " active" : "")}
              onClick={() => onChange(o.value)}>{o.label}{o.sub && <small>{o.sub}</small>}</button>
          ))}
        </div>
      );
    }
    return (
      <div className="gd-select">
        <select value={value} onChange={(e) => {
          ...
          {options.map((o) => (
            <option key={String(o.value)} value={o.value}>{o.label}{o.sub ? " \u00b7 " + o.sub : ""}</option>
          ))}
        </select>
      </div>
    );
  ...
  window.BlyUI = { Field, MoneyInput, NumInput, CostRow, YearSelect, Choice, IcCal, IcPin, IcShield, Chev, Spark };
})();
